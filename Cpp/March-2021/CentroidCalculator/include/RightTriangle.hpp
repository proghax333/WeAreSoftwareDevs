...
class RightTriangle : public virtual Shape, public virtual Body2DProperties, public virtual AngleProperties
{
public:
  static const std::string ShapeType;
  RightTriangle(const Number &width = 0.0, const Number &height = 0.0)
  {
    ...
    std::function<Vector<2>()> getCentroid = [&]() -> Vector<2> {
      ...
      auto result = Vector<2>({x + width / 3, y + height / 3});
...
const std::string RightTriangle::ShapeType = "RightTringle";
